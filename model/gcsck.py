...
"""
GCSCKernel类使用核方法进行光谱聚类。
它首先计算一个邻接矩阵，该矩阵描述数据点之间的相似性。
之后，它使用不同的核函数来计算数据点之间的相似性，并将其转化为一个系数矩阵。
该系数矩阵随后会被后处理，从而得到聚类标签。
该类还包含了几个实用的函数，用于评估聚类的准确性。
"""
...
class GCSCKernel:
    ...
    def __init__(self, n_clusters, regu_coef=1., n_neighbors=10, kernel='rbf', gamma=1., ro=0.5, save_affinity=False):
        ...
        """
        self.n_clusters = n_clusters  # 要形成的聚类数
        self.regu_coef = regu_coef  # 正则化系数
        self.n_neighbors = n_neighbors
        self.kernel = kernel
        self.gamma = gamma
        self.ro = ro
        self.save_affinity = save_affinity
        self.C_f = None  # 初始化 C_final

    # 构造归一化的邻接矩阵。只考虑k近邻图中的连接
