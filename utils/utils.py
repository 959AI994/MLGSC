...
def compute_ppr(a: np.ndarray, alpha=0.2, self_loop=False):
    if self_loop:
...
def compute_adjacent_matrix(features, n_neighbors=30):
    A = kneighbors_graph(features, n_neighbors=n_neighbors, include_self=True).toarray()
    A = A * np.transpose(A)
